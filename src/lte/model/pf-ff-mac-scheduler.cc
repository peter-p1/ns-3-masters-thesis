    ...
    const FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    for (itRach = m_rachList.begin(); itRach != m_rachList.end(); itRach++)
    {
        ...
        if (m_harqOn)
        {
            ...
            std::map<uint16_t, uint8_t>::iterator itProcId =
                m_ulHarqCurrentProcessId.find(uldci.m_rnti);
            std::map<uint16_t, UlHarqProcessesDciBuffer_t>::iterator itDci =
                m_ulHarqProcessesDciBuffer.find(uldci.m_rnti);
            if (itProcId == m_ulHarqCurrentProcessId.end() ||
                itDci    == m_ulHarqProcessesDciBuffer.end())
            {
                // Race: RACH-allocated temp-RNTI not yet registered via
                // CSchedUeConfigReq (or UE just removed). Skip HARQ-DCI
                // bookkeeping; the RAR itself is still emitted. Same
                // tolerant pattern as the UL HARQ feedback path below.
                NS_LOG_WARN("No HARQ buffer for RNTI " << uldci.m_rnti
                            << " yet — skipping HARQ DCI store");
            }
            else
            {
    ...
    const FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    do
    {
        ...
        if (m_harqOn)
        {
            ...
            if (itProcId == m_ulHarqCurrentProcessId.end() ||
                itDci    == m_ulHarqProcessesDciBuffer.end() ||
                itStat   == m_ulHarqProcessesStatus.end())
            {
                // Race: RNTI removed (or never fully registered) between the
                // grant decision above and this HARQ bookkeeping. The DCI is
                // already pushed to ret.m_dciList; just skip HARQ tracking.
                NS_LOG_WARN("No HARQ buffer for RNTI " << uldci.m_rnti
                            << " — skipping HARQ DCI store");
            }
            else
            {
