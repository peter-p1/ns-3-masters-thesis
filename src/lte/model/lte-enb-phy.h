...
class LteEnbPhy : public LtePhy
{
  ...
  public:
    ...
    /**
     * Optional RACH preamble filter for NTN detection models.
     * Callback signature: bool(Ptr<Node> ueNode, Ptr<Node> enbNode)
     * Return true to accept the preamble, false to drop it.
     * When not set (nullptr), all preambles are accepted.
     */
    static std::function<bool(Ptr<Node>, Ptr<Node>)> s_rachPreambleFilter;
