...
class RachPreambleLteControlMessage : public LteControlMessage
{
  public:
    ...
    /**
     * Set the source UE node (for NTN preamble detection models)
     * \param node the UE node that transmitted the preamble
     */
    void SetSourceNode(Ptr<Node> node);

    /**
     * \return the source UE node, or nullptr if not set
     */
    Ptr<Node> GetSourceNode() const;

  private:
    uint32_t m_rapId; ///< the RAPID
    Ptr<Node> m_sourceNode; ///< source UE node (optional, for NTN)
};
