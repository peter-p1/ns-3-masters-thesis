...
std::function<bool(Ptr<Node>, Ptr<Node>)> LteEnbPhy::s_rachPreambleFilter = nullptr;
...
LteEnbPhy::ReceiveLteControlMessageList(std::list<Ptr<LteControlMessage>> msgList)
{
    ...
    for (it = msgList.begin(); it != msgList.end(); it++)
    {
        ...
        case LteControlMessage::RACH_PREAMBLE: {
            ...
            bool accepted = true;
            if (s_rachPreambleFilter)
            {
                Ptr<Node> ueNode = rachPreamble->GetSourceNode();
                Ptr<Node> enbNode = m_netDevice ? m_netDevice->GetNode() : nullptr;
                if (ueNode && enbNode)
                {
                    accepted = s_rachPreambleFilter(ueNode, enbNode);
                }
            }
            if (accepted)
            {
