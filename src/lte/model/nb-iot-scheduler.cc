...
bool NbiotScheduler::s_ceLevelsSet = false;
NbIotRrcSap::NprachParametersNb NbiotScheduler::s_ce0;
NbIotRrcSap::NprachParametersNb NbiotScheduler::s_ce1;
NbIotRrcSap::NprachParametersNb NbiotScheduler::s_ce2;
int NbiotScheduler::s_nprachFormat = 2;  // default Format 2 for NTN
                                ...
                                NbIotRrcSap::SystemInformationBlockType2Nb sib2)
{
  ...
  m_sib2config = sib2;
  m_sib2config.radioResourceConfigCommon.nprachConfig.nprachParametersList.nprachParametersNb0.numRepetitionsPerPreambleAttempt = m_ce0.numRepetitionsPerPreambleAttempt;
  m_sib2config.radioResourceConfigCommon.nprachConfig.nprachParametersList.nprachParametersNb1.numRepetitionsPerPreambleAttempt = m_ce1.numRepetitionsPerPreambleAttempt;
  m_sib2config.radioResourceConfigCommon.nprachConfig.nprachParametersList.nprachParametersNb2.numRepetitionsPerPreambleAttempt = m_ce2.numRepetitionsPerPreambleAttempt;
  m_DciTimeOffsetRmaxSmall.reserve (8);
  m_DciTimeOffsetRmaxBig.reserve (8);
  m_Msg3TimeOffset.reserve (4);
  m_logdir = "";
  if (!s_globalLogDir.empty())
    {
      m_logdir = s_globalLogDir;
    }
  const char* envLog = std::getenv("NBIOT_LOGDIR");
  if (envLog && m_logdir.empty())
    {
      m_logdir = std::string(envLog);
    }
  ...
  {
    NbIotRrcSap::NpdcchMessage m;
    m.isRar = true;
    m.ce = m_ce0.coverageEnhancementLevel;
    if (m.ce == m_ce0.coverageEnhancementLevel)
      {
        m.dciN0.dciRepetitions = NbIotRrcSap::DciN0::DciRepetitions::r2;
        m.dciN1.dciRepetitions = NbIotRrcSap::DciN1::DciRepetitions::r2;
      }
    LogRarRepetitions(m, ConvertNprachParametersNb2SearchSpaceConfig(m_ce0));
  }
  {
    NbIotRrcSap::NpdcchMessage m;
    m.isRar = true;
    m.ce = m_ce1.coverageEnhancementLevel;
    if (m.ce == m_ce1.coverageEnhancementLevel)
      {
        m.dciN0.dciRepetitions = NbIotRrcSap::DciN0::DciRepetitions::r32;
        m.dciN1.dciRepetitions = NbIotRrcSap::DciN1::DciRepetitions::r32;
      }
    LogRarRepetitions(m, ConvertNprachParametersNb2SearchSpaceConfig(m_ce1));
  }
  {
    NbIotRrcSap::NpdcchMessage m;
    m.isRar = true;
    m.ce = m_ce2.coverageEnhancementLevel;
    if (m.ce == m_ce2.coverageEnhancementLevel)
      {
        m.dciN0.dciRepetitions = NbIotRrcSap::DciN0::DciRepetitions::r256;
        m.dciN1.dciRepetitions = NbIotRrcSap::DciN1::DciRepetitions::r256;
      }
    LogRarRepetitions(m, ConvertNprachParametersNb2SearchSpaceConfig(m_ce2));
  }
  ...
  if (m_only15KhzSpacing)
    {
      ...
      for (std::vector<NbIotRrcSap::NprachParametersNb>::iterator it = ces.begin ();
           it != ces.end (); ++it)
        {
          ...
          // NPRACH timing parameters depend on preamble format:
          //   Format 0: 3.75 kHz spacing, CP =  66.7 us (cell radius <=  10 km)
          //   Format 1: 3.75 kHz spacing, CP = 266.7 us (cell radius <=  40 km)
          //   Format 2: 1.25 kHz spacing, CP = 800.0 us (cell radius <= 120 km, NTN)
          // See 3GPP TS 36.211 Section 10.1.6
          double cpMs;
          double preambleSymbolTime;
          if (m_nprachFormat == 2)
            {
              // Format 2 (Release 15): 1.25 kHz subcarrier spacing
              // Symbol time = 1/(1.25 kHz) = 0.8 ms; with samples: 24576 * Ts
              double ts = 1000.0 / (15000.0 * 2048.0);  // basic time unit [ms]
              preambleSymbolTime = 24576.0 * ts;         // ~0.8 ms
              cpMs = 0.8;                                 // 800 us CP
            }
          else
            {
              // Format 0/1: 3.75 kHz subcarrier spacing
              double ts = 1000.0 / (15000.0 * 2048.0);
              preambleSymbolTime = 8192.0 * ts;          // ~0.2667 ms
              cpMs = (m_nprachFormat == 0) ? 0.0667 : 0.266;
            }
          double preambleGroupTimeNoCP = 5.0 * preambleSymbolTime;
          double preambleGroupTime = cpMs + preambleGroupTimeNoCP;
          double preambleRepetition = 4.0 * preambleGroupTime;
          int numReps = NbIotRrcSap::ConvertNumRepetitionsPerPreambleAttempt2int (*it);
          double nprachduration = numReps * preambleRepetition;

          // TS 36.211 Sec. 10.1.6: Format 2 inserts a 40 ms gap after every
          // 16 preamble repetitions (i.e. every 64 symbol groups).
          if (m_nprachFormat == 2 && numReps > 16)
            {
              int numGaps = (numReps - 1) / 16;   // gaps *between* blocks of 16
              nprachduration += numGaps * 40.0;    // 40 ms per gap
            }
          ...
          // NPRACH IE values are in native subcarrier units (TS 36.331):
          //   Format 0/1 (NPRACH-Parameters-NB-r13):    3.75 kHz units
          //   Format 2   (NPRACH-ParametersFmt2-NB-r15): 1.25 kHz units
          // Map to the 15 kHz uplink grid (12 subcarriers in 180 kHz):
          //   3.75 kHz -> 15 kHz : divide by 4  (4 *  3.75 kHz = 15 kHz)
          //   1.25 kHz -> 15 kHz : divide by 12 (12 * 1.25 kHz = 15 kHz)
          const size_t scsScale = (m_nprachFormat == 2) ? 12u : 4u;
              ...
              if (window_condition == 0)
                {
                  ...
                  for (size_t j = 0; j < time_tmp; ++j)
                    {
                      for (size_t k = 0; k < numberSubcarriers / scsScale; ++k)
                        {
                          m_uplink[subcarrierOffset / scsScale + k][i + j] = -1;
                        }
                    }
                  i += time_tmp;
                }
  ...
  if (IsSearchSpaceBegin (ConvertNprachParametersNb2SearchSpaceConfig (m_ce0)))
    {
      ...
      {
        NbIotRrcSap::NpdcchMessage probe;
        probe.isRar = true;
        probe.ce = currentSearchSpace.ce;
        if (probe.ce == m_ce0.coverageEnhancementLevel)
          {
            probe.dciN0.dciRepetitions = NbIotRrcSap::DciN0::DciRepetitions::r2;
            probe.dciN1.dciRepetitions = NbIotRrcSap::DciN1::DciRepetitions::r2;
          }
        else if (probe.ce == m_ce1.coverageEnhancementLevel)
          {
            probe.dciN0.dciRepetitions = NbIotRrcSap::DciN0::DciRepetitions::r32;
            probe.dciN1.dciRepetitions = NbIotRrcSap::DciN1::DciRepetitions::r32;
          }
        else if (probe.ce == m_ce2.coverageEnhancementLevel)
          {
            probe.dciN0.dciRepetitions = NbIotRrcSap::DciN0::DciRepetitions::r256;
            probe.dciN1.dciRepetitions = NbIotRrcSap::DciN1::DciRepetitions::r256;
          }
        LogRarRepetitions(probe, currentSearchSpace);
      }
...
void
NbiotScheduler::LogRarRepetitions(const NbIotRrcSap::NpdcchMessage& msg, const SearchSpaceConfig& ssc)
{
  if (m_logdir.size() == 0)
    {
      if (!s_globalLogDir.empty())
  ...
  std::string logfile_path = m_logdir + "ntn-lena-nb-traces.txt";
  std::ofstream ofs(logfile_path.c_str(), std::ios_base::app);
  if (!ofs.is_open())
    {
      return;
    }
  uint16_t dciN0Reps = NbIotRrcSap::ConvertDciN0Repetitions2int(msg.dciN0);
  uint16_t dciN1Reps = NbIotRrcSap::ConvertDciN1Repetitions2int(msg.dciN1);
  uint16_t nprachReps = NbIotRrcSap::ConvertNumRepetitionsPerPreambleAttempt2int(
      (ssc.ce == NbIotRrcSap::NprachParametersNb::CoverageEnhancementLevel::zero ? m_ce0 :
       ssc.ce == NbIotRrcSap::NprachParametersNb::CoverageEnhancementLevel::one ? m_ce1 : m_ce2));
  uint16_t npdcchRaReps = NbIotRrcSap::ConvertNpdcchNumRepetitionsRa2int(
      (ssc.ce == NbIotRrcSap::NprachParametersNb::CoverageEnhancementLevel::zero ? m_ce0 :
       ssc.ce == NbIotRrcSap::NprachParametersNb::CoverageEnhancementLevel::one ? m_ce1 : m_ce2));

  ofs << "[RAR_REPETITIONS] Time: " << Simulator::Now().GetSeconds()
      << "s, CE: " << (ssc.ce == NbIotRrcSap::NprachParametersNb::CoverageEnhancementLevel::zero ? 0 :
                        ssc.ce == NbIotRrcSap::NprachParametersNb::CoverageEnhancementLevel::one ? 1 : 2)
      << ", DCI-N0: " << dciN0Reps
      << ", DCI-N1: " << dciN1Reps
      << ", NPRACH: " << nprachReps
      << ", NPDCCH-RA: " << npdcchRaReps
      << std::endl;
  ofs.close();
}
      ...
      for (auto &i : m_HarqTimeOffsets)
        {
          for (size_t j = 0; j < 4; ++j)
            { // NPUSCH single-tone subcarriers 0-3 (15 kHz spacing); independent of NPRACH format
              uint64_t candidate =
      ...
      for (auto &i : m_DciTimeOffsetUplink)
        {
          for (size_t j = 0; j < 4; ++j)
            { // NPUSCH single-tone subcarriers 0-3 (15 kHz spacing); independent of NPRACH format
              ...
              tmp.numResourceUnits = NbIotRrcSap::DciN0::NumResourceUnits::ru1;
              tmp.numNpuschRepetitions = NbIotRrcSap::DciN0::NumNpuschRepetitions::r1;
              tmp.mCS = NbIotRrcSap::DciN0::MCS::one;
              tmp.tbs = 0;
...
namespace ns3 {
std::string NbiotScheduler::s_globalLogDir = std::string();

void
NbiotScheduler::SetGlobalLogDir(const std::string& logdir)
{
  s_globalLogDir = logdir;
}

void
NbiotScheduler::SetGlobalCeLevels(NbIotRrcSap::NprachParametersNb ce0,
                                  NbIotRrcSap::NprachParametersNb ce1,
                                  NbIotRrcSap::NprachParametersNb ce2)
{
  s_ce0 = ce0;
...
void
NbiotScheduler::SetGlobalNprachFormat(int format)
{
  s_nprachFormat = format;
}
} // namespace ns3
