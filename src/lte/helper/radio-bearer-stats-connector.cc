                                             ...
                                             uint16_t rnti)
{
    ...
    if (it == m_ueManagerPathByCellIdRnti.end())
    {
        // Race: SRB0 trace fired before NotifyNewUeContext registered the
        // (cellId,rnti) -> ueManager path. Skip wiring stats for this UE.
        NS_LOG_WARN("ConnectTracesSrb0: no UE-manager path for (cellId="
                    << cellId << ", rnti=" << rnti << ") — skipping RLC stats wiring");
        return;
    }
                                             ...
                                             uint16_t rnti)
{
    ...
    if (it == m_ueManagerPathByCellIdRnti.end())
    {
        // Race: SRB1 trace fired before NotifyNewUeContext registered the
        // (cellId,rnti) -> ueManager path. Skip wiring stats for this UE.
        NS_LOG_WARN("ConnectTracesSrb1: no UE-manager path for (cellId="
                    << cellId << ", rnti=" << rnti << ") — skipping RLC/PDCP stats wiring");
        return;
    }
    std::string ueManagerPath = it->second;
    NS_LOG_LOGIC("ueManagerPath = " << ueManagerPath);
